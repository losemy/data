...
@Lazy
@Service
@RocketMQMessageListener(topic = "${demo.rocketmq.orderUpdTopic}",
        consumerGroup = "${demo.rocketmq.orderUpdTopic}", consumeMode = ConsumeMode.CONCURRENTLY)
@Slf4j
public class OrderUpdConsumer implements RocketMQListener<Order>, RocketMQPushConsumerLifecycleListener {
    ...
    public void onMessage(Order message) {
        try {
            log.info("upd-message {}", JSON.toJSONString(message));
            ...
            log.info("handler-update result {}", result);
            if(result <= 0 ){
                //发送延迟消息
                //丢弃旧的消息
                ...
                if(result <= 0) {
                    ...
                    log.info("handler-upd-later {}", JSON.toJSONString(message));
                    ...
                    log.info("handler-upd-remove {}",JSON.toJSONString(message));
                }
            }
