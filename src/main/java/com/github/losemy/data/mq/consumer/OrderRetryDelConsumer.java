...
@Lazy
@Service
@RocketMQMessageListener(topic = "${demo.rocketmq.orderRetryDelTopic}",
        consumerGroup = "${demo.rocketmq.orderRetryDelTopic}", consumeMode = ConsumeMode.CONCURRENTLY)
@Slf4j
public class OrderRetryDelConsumer implements RocketMQListener<Order>, RocketMQPushConsumerLifecycleListener {
    @Autowired
    ...
    public void onMessage(Order message) {
        ...
        log.info("retry-del-result {} retry-del-message {}",result,JSON.toJSONString(message));
        ...
        if(result <=0){
            ...
            if(result <=0){
                ...
                // 还没有完成老数据的同步， 数据就被删除了，此时是不可能消费掉的（只有del消费，没有数据会到新库）
                //等待同步任务彻底完成 1. id < 同步maxId 2. 同步完成
                if(syncData.isSyncDataFinish() && message.getId() < syncData.getMaxId()) {
                    log.info("handler-retry-delete-remove-for-all-ready-deleted {}",JSON.toJSONString(message));
                }else{
                    Message<Order> delayMessage = MessageBuilder.withPayload(message).build();
                    log.info("handler-retry-delete-later {}", JSON.toJSONString(message));
                ...
                log.info("handler-retry-delete-remove {}",JSON.toJSONString(message));
            }
        }
